...
//* courses made unique by their code.
export async function getCourseClassInfoList(req: Request, res: Response< Record<string, Record<number, CourseDisplayInfo[]>>>) {
    const courseDisplayInfoList = timetableService.getUniqueCoursesByCode();
    res.send(courseDisplayInfoList);
}
